...
npzFiles = glob(data_path+'*.npz')[:20]
util.set_subjects_info(npzFiles)
...
for fold, (train_idx, test_idx) in enumerate(skf.split(data, labels)):
    ...
    util.run(model, train_data, train_labels, test_data, test_labels, BATCH_SIZE, EPOCHS, learn_rate=1e-2)
    # 只测试一次，不进行完整的交叉验证
    break
